...
app.route('/informacion').get(function(req, res){
	var jsonvar = '{';
	let total_usuarios = await db.collection('mensajes').aggregate([{$group: {_id: "$usuario"}}]).toArray(async function(err, resultado){
		if(err) throw res.status(400).send("No se pudo conectar a la base de datos.");
		jsonvar += '"total_usuarios": ' + resultado.length + ",";
		console.log(jsonvar);
	});     
    let usuario_mayor = await db.collection('mensajes').aggregate([{$group: {_id: "$usuario", count: {$sum: 1}}}]).sort({count: -1}).toArray(async function(err, resultado){
		if(err) throw res.status(400).send("No se pudo conectar a la base de datos.");
